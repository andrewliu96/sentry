...
from sentry.utils.safe import safe_execute, trim, trim_dict, get_path, get_valid
...
def generate_culprit(data, platform=None):
    exceptions = get_valid(data, 'exception', 'values')
    ...
    if stacktraces:
        ...
        culprit = get_valid(data, 'request', 'url') or ''
...
class EventManager(object):
    ...
    def should_filter(self):
        ...
        for exception_interface in get_valid(self._data, 'exception', 'values') or []:
            message = u': '.join(
